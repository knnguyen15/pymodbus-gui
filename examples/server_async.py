...
def setup_server(description=None, context=None, cmdline=None):
    ...
    if not args.context:
        ...
        if args.store == "sequential":
            ...
            datablock = lambda : ModbusSequentialDataBlock(0x00, range(300))  # pylint: disable=unnecessary-lambda-assignment
            datablock_co = lambda : ModbusSequentialDataBlock(0x00, [0, 1] * 500)  # pylint: disable=unnecessary-lambda-assignment
        ...
        if args.slaves > 1:
            ...
            context = ModbusSlaveContext(
                di=datablock_co(), co=datablock_co(), hr=datablock(), ir=datablock()
            )
            single = True
